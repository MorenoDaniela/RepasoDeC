...
int inf_orquestasMenosDeSeisMusicos(Musico* arrayMusico,int limiteMusico, Orquesta* arrayOrquesta, int limiteOrquesta)
{
    ...
    if (arrayMusico!=NULL && arrayOrquesta!=NULL && limiteMusico>0 && limiteOrquesta>0)
    {
        for (i=0;i<limiteOrquesta;i++)
        {
            ...
            if (arrayOrquesta[i].isEmpty==0)
            {
                if (musico_buscarID(arrayMusico,limiteMusico,arrayOrquesta[i].idOrquesta,&posMusico)==0)
                {
                    if(arrayMusico[posMusico].idOrquesta==arrayMusico[posMusico+1].idOrquesta)
                    {
                        contador++;
                    }
                    if (arrayMusico[posMusico].idOrquesta!=arrayMusico[posMusico+1].idOrquesta && contador<6)
                    {
                        contador=0;
                        //if (contador<6)
                          //  {
